...
if len(sys.argv) < 5:
        print("Usage: python/python3 feature_extract_tvz.py IMPORT_DATABASE LIMIT_START_INDEX LIMIT_NUM OUTPUT_FILE_NAME")
        sys.exit(-1)
...
def get_features(race, player_replay_ID, replayID, bottom_frame, upper_frame, stats, oppo_stats):
    ...
    ## building_slots
    building_slots = ([0] * len(TERRAN_BUILDING_ID_LIST)) if race == Race.Terran else ([0] * len(ZERG_BUILDING_ID_LIST))
    ...
    if building_total_num != 0:
        if race == Race.Terran:
            ...
            for building in stats['current_buildings']:
                for i in range(len(ZERG_BUILDING_ID_LIST)):
                    if building[2] == ZERG_BUILDING_ID_LIST[i]:
                        building_slots[i] += 1
        else:
            raise ValueError

    ## unit_slots
    unit_slots = ([0] * len(TERRAN_UNIT_ID_LIST)) if race == Race.Terran else ([0] * len(ZERG_UNIT_ID_LIST))
    ...
    if unit_total_num != 0:
        if race == Race.Terran:
            ...
            for unit in stats['current_units']:
                for i in range(len(ZERG_UNIT_ID_LIST)):
                    if unit[2] == ZERG_UNIT_ID_LIST[i]:
                        unit_slots[i] += 1
        else:
            raise ValueError
